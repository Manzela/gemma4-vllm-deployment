# Gemma 4 26B-A4B-it (MoE) — vLLM Deployment on Vertex AI
...
[![Model: Gemma 4](https://img.shields.io/badge/model-Gemma_4_26B--A4B--it-4285F4?logo=google)](https://huggingface.co/google/gemma-4-26B-A4B-it)

> **Forensic documentation of deploying Google's Gemma 4 26B-A4B-it Mixture-of-Experts model on Vertex AI with vLLM — 20 distinct failure modes discovered, the unsolvable dependency triangle proven, and a community call-to-action for Multi-LoRA, NF4 quantization, vision, and thinking support.**

---

## Author & Credits

**Daniel Manzela** — Independent research conducted April 2026 during production deployment of the Gemma 4 26B-A4B-it (MoE, 4B active parameters) model on Google Cloud Vertex AI.
...
Deploying **Gemma 4 26B-A4B-it** — Google's latest Mixture-of-Experts (MoE) model with 26 billion total parameters (4 billion active) — on Vertex AI with vLLM is significantly harder than it should be.
...
| Parameter | Value |
|---|---|
| **Model** | `google/gemma-4-26b-a4b-it` (MoE, 4B active) |
...
├── docs/
│   ├── FORENSIC\_RUNBOOK.md        \# 20 failure modes documented
│   ├── DEPENDENCY\_MATRIX.md       \# The unsolvable constraint proof
│   ├── DEPLOYMENT\_GUIDE.md        \# Step-by-step deployment
│   └── KNOWN\_LIMITATIONS.md       \# Current blockers
│
├── container/
│   ├── Dockerfile                 \# GCSFUSE-enabled vLLM container
│   ├── entrypoint.sh              \# Tokenizer patching + vLLM startup
│   └── chat\_template.jinja        \# Simplified Gemma 4 chat template
│
├── deployment/
│   ├── upload\_model.py            \# Model registration & deployment script
│   └── deploy\_config.env.example  \# Environment variable reference
│
├── logs/
│   ├── cycle1\_base\_deployment.md  \# v1–v30 deployment log
│   └── cycle2\_lora\_enablement.md  \# LoRA enablement attempts
│
├── CHANGELOG.md
...
| 18 | `is_offline_mode` import | huggingface\_hub version mismatch chain |
...
👉 **Full details with error messages, stack traces, and fixes:** [Forensic Runbook](https://www.google.com/search?q=docs/FORENSIC_RUNBOOK.md)
...
See the [Deployment Guide](https://www.google.com/search?q=docs/DEPLOYMENT_GUIDE.md) for complete instructions. The minimal steps:
...
👉 **Mathematical proof and visual diagram:** [Dependency Matrix](https://www.google.com/search?q=docs/DEPENDENCY_MATRIX.md)
...
*This repository is maintained by [Daniel Manzela](https://github.com/Manzela). All findings and documentation are original work. Contributions from the community are welcome — see [CONTRIBUTING.md](https://www.google.com/search?q=CONTRIBUTING.md).*
